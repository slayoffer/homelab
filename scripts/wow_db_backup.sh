...
for db in $DBS; do
    ...
    cd ~/dev/scripts
    sudo docker exec $DB_CONTAINER /usr/bin/mysqldump -u root -p$MSQL_ROOT_PWD --databases $db > wow-$db-$(date +"%Y%m%d%H").sql
    sudo gzip wow-$db-$(date +"%Y%m%d%H").sql
    sudo mv wow-$db-$(date +"%Y%m%d%H").sql.gz $BACKUPDIR
done
